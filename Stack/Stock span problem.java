...
class Solution {
    public ArrayList<Integer> calculateSpan(int[] arr) {
       ...
       Stack<Integer> st = new Stack<>();
        ...
        {
            // remove the less than value s from the stack
            while(!st.isEmpty()  && arr[st.peek()] <= arr[i] )
            {
                // we will pop  the elem from the  stwvck
                st.pop();
            }
            //if the stack is empty
            if(st.isEmpty())
            {
                // mainy for the first index
                // no prev high
                ans.add(i+1); // span is i+1
            }
            // for the valid value
            //add  the span to the ans
            else
            {
                ans.add(i - st.peek());
            }
            // for all the case we will push the arr[i]
            st.push(i);
        }
        return ans;
